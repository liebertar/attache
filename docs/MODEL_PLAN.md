...
| **Super** | Nemotron 3 Super 120B-A12B (Mar 2026) `nvidia/nemotron-3-super-120b-a12b` | 런타임 옆(호출만, 호스팅 안 함). 로컬 대역: 11434 의 `nemotron-3-nano:latest`(30B), 런타임 프로세스에만 | ① **관제 권고**(같은 막힘 3번째 거절에 코드가 만든 합법 선택지 중 하나 + 400자 요약) ② 문법으로 못 읽는 NOTAM 문장을 규칙 스키마로 구조화 → **사람 확인 뒤** 적용 ③ **정보 수집 읽기**: 문법이 못 읽는 날씨·사고·제한 글(Tavily 검색 결과, 수동 입력)을 `{kind: weather|incident|notice|none, …}` 로 — 숫자·주소·반경·창은 코드가 검사하고(지명 사전 밖 주소는 폐기), 통과한 것도 사람 확인 뒤에만 적용 | 설명·초안. 목록 밖 답은 버림. 규칙을 조이는 쪽만 즉시, 푸는 쪽은 사람 | ①② **코드 됨.** 로컬 대역 라이브: 공지(MEDEVAC 문장)를 8.8 s 에 읽어 보류 → 사람 없이 lapsed(수정 전엔 4판 중 1판, 20 s 타임아웃); 권고는 괄호 관용 뒤 `source: super`(수정 전 160 중 155 가 `"[hold]"` 로 규칙 폴백). 대역이 0.5 NM 원을 잘못 놓아 승인→회수는 authored fixture 로만. **Nebius 120B 미검증(키 없음)** |
...
| **기상 대기** | 날씨 보고서(METAR 어투·예보 산문·검색 결과)를 문법이 m/s·m 로 읽어 `fleet.yaml weather` 한도(바람 10·돌풍 12 m/s·시정 1500 m)와 비교. 관제탑 피드(시뮬레이터 공지)가 넘으면 그 틱에 정책 셋(`weather-hold:fly_route/reserve_pad/depart`, 땅에 있는 기체만) + 아직 안 뜬 승인 경로 물림; 검색 결과·수동 입력은 문법이 읽어도 사람 카드 뒤. 떠 있는 기체는 내림. 한도 안 보고서로는 안 풀리고 사람(`lift_weather_hold` 카드) 또는 창 만료로만 | `WEATHER HOLD · takeoffs held until tick 2700`, 배너 `read by the rule grammar` | 됨 (`core/intake.py`, `runtime/intake.py`; 씨앗 7 런타임 0 / 직결 2 이륙) |
| **사고 접근 금지** | 주소 또는 건물 id 가 있는 사고 문장 → 지명 사전에서 자리 → 반경(기본 150 m) 원 Volume 을 공지 책에. 지나던 회랑 회수(nearest_exit), 새 경로 거절, 원 안·둘레 50 m 착륙장 착륙 불가. 모델이 읽은 것은 사람 확인 뒤 | `NO ROOM TO LAND · FIRE · 4705 Center Boulevard`, 배너 `150 m keep-out until tick 3600` | 됨 (씨앗 7 에서 drone-02 갠트리행 회랑 회수 1·거절 1) |
