...
## E0-2 · 한 기체에 한 번에 한 선만

지금 같은 기체에 `AWAITING` 선이 두 개씩 그려집니다. 거절 구간은 큐에 넣는데
승인 구간은 즉시 시작하도록 해놔서, 둘이 겹칩니다.

- `stageRoute()`: **모든 구간을 기체별 큐에 넣을 것.** 다음 구간 시작 시각은
  `거절: GROW+CHECK+HOLD` (흐려지기 시작할 때) / `승인: stageLife`.
- 그러면 재생이 길어져 기체가 승인 재생보다 먼저 출발합니다.
  `sim/world.py CLEARANCE_TICKS` 를 그만큼 올릴 것(거절 한 번이 앞에 붙는 경우
  7.2초 + 6.8초 = 14초 ≈ 70틱).
...
| 시나리오 | 어디에 | 상태 |
|---|---|---|
| 시작 화면: 네 대가 창고에서 짐 싣는 중 | E1, E4 | 안 됨 |
