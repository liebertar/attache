...
image, and it does not use `holdshort.adapters` — a team that wires an agent straight to the
vehicle writes its own client, which is exactly why there is no shared place to put a rule.

It shares `holdshort.agent`'s eyes and hands: the same detector, the same proposal writer,
